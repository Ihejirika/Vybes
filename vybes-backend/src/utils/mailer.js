...
const sendTicketEmail = async (email, eventTitle, tierName, qrHash, ticketCode) => {
    ...
    try {
        const { data, error } = await resend.emails.send({
            from: process.env.MAIL_FROM || 'Vybe Ticketing <onboarding@resend.dev>',
            to: email,
        ...
        if (error) {
            console.error('Resend error during webhook:', error);
            throw error;
        }

        console.log(`Ticket email sent to ${email} (id: ${data?.id})`);
    } catch (err) {
        console.error('Mail error during webhook:', err.message || err);
        throw err;
    }
};
